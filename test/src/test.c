...
static void test_create_and_write(void)
{
    ...
    int fd = _open(TEST_FILE, O_CREAT | O_TRUNC | O_RDWR | O_BINARY, 0644);
    expect(fd >= 0, "open O_CREAT | O_TRUNC");
...
static void test_read_back(void)
{
    ...
    int fd = _open(TEST_FILE, O_RDONLY | O_BINARY);
    expect(fd >= 0, "open O_RDONLY");
...
static void test_truncate(void)
{
    ...
    int fd = _open(TEST_FILE, O_TRUNC | O_RDWR);
    expect(fd >= 0, "open O_TRUNC");
...
static void test_append(void)
{
    ...
    fd = _open(TEST_FILE, O_APPEND | O_RDWR | O_BINARY);
    expect(fd >= 0, "open O_APPEND");

    _write(fd, "DEF", 3);
    _close(fd);

    fd = _open(TEST_FILE, O_RDONLY | O_BINARY);
    char buf[16] = {0};
...
static void test_text_mode_translation(void)
{
    ...
    int fd = _open(TEST_FILE, O_CREAT | O_TRUNC | O_RDWR | O_TEXT, 0644);
    expect(fd >= 0, "open O_TEXT");

    _write(fd, "A\nB\n", 4);
    _close(fd);

    fd = _open(TEST_FILE, O_RDONLY | O_TEXT);
    char buf[16] = {0};
    int r = _read(fd, buf, sizeof(buf));
...
static void test_error_conditions(void)
{
    ...
    int fd = _open(TEST_FILE, O_RDONLY);
    ...
    fd = _open("D:/does_not_exist.txt", O_RDONLY);
